    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `ShoppingListItem` (`id` INTEGER NOT NULL, `name` TEXT NOT NULL, `quantity` INTEGER NOT NULL, PRIMARY KEY (`id`))');
...
class _$ShoppingListItemDAO extends ShoppingListItemDAO {
  ...
  @override
  Future<List<ShoppingListItem>> findAllShoppingListItems() async {
    return _queryAdapter.queryList('SELECT * FROM ShoppingListItem',
        mapper: (Map<String, Object?> row) => ShoppingListItem(
            row['id'] as int, row['name'] as String, row['quantity'] as int));
  }

  @override
  Future<void> insertShoppingListItem(ShoppingListItem shopListItem) async {
    await _shoppingListItemInsertionAdapter.insert(
        shopListItem, OnConflictStrategy.abort);
  }

  @override
  Future<void> deleteShoppingListItem(ShoppingListItem shopListItem) async {
    await _shoppingListItemDeletionAdapter.delete(shopListItem);
  }
}
